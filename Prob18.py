...
# Recursive method which adds the current number to the sum of all the paths below it,
# then passes the array of sums up to the instance of the sum method that called it
def sum(row, column):
    numHere = pyramid[row][column]
    if row == len(pyramid) - 1:
        return [numHere]

    sumDown = sum(row + 1, column)
    sumDiag = sum(row + 1, column + 1)

    sumBoth = []

    for num in sumDown:
        sumBoth.append(num + numHere)
